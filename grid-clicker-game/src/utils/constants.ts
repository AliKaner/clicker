...
// Grid configuration
export const DEFAULT_GRID_SIZE = { width: 16, height: 8 };
...
export const BALANCE = {
  ...
  getClickUpgradeCost: (level: number) =>
    CLICK_UPGRADE_BASE_COST * Math.pow(2, level),

  // Cell assignment cost formula: base * 2^level
  getCellAssignCost: (level: number) =>
    CELL_ASSIGN_BASE_COST * Math.pow(2, level),

  // Cell upgrade cost formula: base * 2^level
  getCellUpgradeCost: (level: number) =>
    CELL_ASSIGN_BASE_COST * Math.pow(2, level),
  ...
  getColorUpgradeCost: (level: number) =>
    CELL_ASSIGN_BASE_COST * 5 * Math.pow(2, level),
} as const;

// Visual effect formulas
export const VISUAL_EFFECTS = {
  // Button border width: min(2 + level, 6)
  getButtonBorderWidth: (level: number) =>
    Math.min(BASE_BORDER_WIDTH + level, MAX_BORDER_WIDTH),

  // Button glow size: 8 + level * 4
  getButtonGlowSize: (level: number) =>
    BASE_GLOW_SIZE + level * GLOW_MULTIPLIER,
  ...
  getCellBorderWidth: (level: number) =>
    Math.min(BASE_BORDER_WIDTH + level, MAX_BORDER_WIDTH),
